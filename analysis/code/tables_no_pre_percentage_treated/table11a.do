...
replace percent_treated_`distance'_otherrace = ((treated_`distance'_b + treated_`distance'_n + treated_`distance'_o + treated_`distance'_h)/ (treated_`distance'_b + treated_`distance'_n + treated_`distance'_o + treated_`distance'_h +  control_`distance'_b + control_`distance'_n + control_`distance'_other + control_`distance'_h))*100 if race == "White Non-Hispanic"
...
file open file9 using "$repository/analysis/tables/tables_no_pre_percentage_treated/table11a_races_own_other_race.tex", replace write
...
file write file9 _n "\caption{Spillover Own Race and Other Race} {\label{tab:results_hispanics} \begin{threeparttable}"
...
foreach d of local distance  {	
	...
	***FIXED EFFECTS - ALL RACES
	quietly xtreg std_`assess' percent_treated_`d'_ownrace  percent_treated_`d'_otherrace i.test_num  if has_`assess' == 1, fe cluster(child) 
	
	matrix d = r(table) 

	*adding stars
		if d[4,1] < 0.01 {
			local item_`j'_`d' = string(round(_b[percent_treated_`d'_ownrace], .0001), "%13.0gc") + "***"
		}
		...
		local se_`j'_`d' = string(round(_se[percent_treated_`d'_ownrace], .0001), "%13.0gc")
		
		local j = `j' + 1

	
	***FIXED EFFECTS - ONLY SUBSAMPLE BLACKS
	quietly xtreg std_`assess' percent_treated_`d'_ownrace  percent_treated_`d'_otherrace i.test_num  if has_`assess' == 1 & race == "African American", fe cluster(child) 
	...
	***FIXED EFFECTS - ONLY SUBSAMPLE HISPANICS
	quietly xtreg std_`assess' percent_treated_`d'_ownrace  percent_treated_`d'_otherrace i.test_num  if has_`assess' == 1 & race == "Hispanic", fe cluster(child) 
...
foreach d of local distance  {	
	...
	*adding stars
		if d[4,1] < 0.01 {
			local item_`j'_`d' = string(round(_b[percent_treated_`d'_otherrace], .0001), "%13.0gc") + "***"
		}
		else if d[4,1] < 0.05 & d[4,1] >= 0.01 {
			local item_`j'_`d' = string(round(_b[percent_treated_`d'_otherrace], .0001), "%13.0gc") + "**"
		}	
		...
		local se_`j'_`d' = string(round(_se[percent_treated_`d'_otherrace], .0001), "%13.0gc")
